...
var wpdtrt_plugin_boilerplate_admin_ui = {
    ...
    ajax_init: function ajax_init($) {
        ...
        if (config.refresh_api_data === "true") {
            $ajax_container_data = $("." + abbr + "-ajax-response[data-format=\"data\"]");
            $ajax_container_ui = $("." + abbr + "-ajax-response[data-format=\"ui\"]");

            $ajax_container_data.empty().append("<div class=\"spinner is-active\">" + loading_message + "</div>");

            $ajax_container_ui.empty().append("<div class=\"spinner is-active\">" + loading_message + "</div>");

            data = $.post(ajaxurl, {
                "action": prefix + "_refresh_api_data",
                "format": "ui"
            }, function (response) {
                $ajax_container_ui.empty().html(response);
            });
            ...
            data.done(function () {
                $.post(ajaxurl, {
                    ...
                    $ajax_container_data.empty().html(response);
                });
            });
        }
    }
